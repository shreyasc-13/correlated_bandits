...
def parse_arguments():
    ...
    parser.add_argument('--movielens', dest='process_movielens', action='store_true', default=True, help="Pre-process MovieLens data")
    parser.add_argument('--goodreads', dest='process_goodreads', action='store_true', default=True, help="Pre-process Goodreads data")
    parser.add_argument('--train_test_ratio', dest='train_test_ratio', type=float, default=0.5, help="Train test split ratio")
    ...
    for table_movie in range(50):
        ...
        for rating in range(1,6):
            ...
            for movie in np.delete(np.arange(0,50), table_movie):
                d = train_data[(train_data['Movie_Assigned_ID'] == movie) & \
                                  (train_data['UserID'].isin(users))]

                if d['Rating'].shape[0] != 0:
                    movie_tables[table_movie][rating-1, movie] = np.average(d['Rating']) #Note: Use 'rating-1' to index
                else:
...
def book_train_test_split(train_test_ratio):

    data = pd.read_csv('goodreads_poetry_cropped.csv')
    ...
    for i in range(len(reverse_sorted_users)):
        cnt += reverse_sorted_users[i][0]
        users_for_train.append(reverse_sorted_users[i][1])
        if s > train_test_ratio * chosen_data.shape[0]:
            break
...
def book_data_process():
    ...
    pickle.dump(true_means, open('books/true_means_test', 'w'))
